...
import com.social.backend.post.PostService;
...
public class PostControllerTest {
	...
	private static final String API_V1_POSTS = "/api/v1/posts";
	...
	@Test
	public void postPost_whenPostHasFileAttachmentAndUserIsAuthorized_fileAttachmentPostRelationIsUpdatedInDatabase()
			throws IOException {
		userService.save(TestUtil.createValidUser("user1"));
		authenticate("user1");
	...
	@Test
	public void postPost_whenPostHasFileAttachmentAndUserIsAuthorized_postFileAttachmentRelationIsUpdatedInDatabase()
			throws IOException {
		userService.save(TestUtil.createValidUser("user1"));
		authenticate("user1");
	...
	public void getPostsOfUser_whenUserExists_receivePageWithZeroPosts() {
		userService.save(TestUtil.createValidUser("user1"));
		ResponseEntity<TestPage<Object>> response = getPostsOfUser("user1",
	...
	public void getPostsOfUser_whenUserExistWithPost_receivePageWithPostVM() {
		...
		ResponseEntity<TestPage<PostVM>> response = getPostsOfUser("user1",
				new ParameterizedTypeReference<TestPage<PostVM>>() {
				});
		PostVM storedPost = response.getBody().getContent().get(0);
		assertThat(storedPost.getUser().getUsername()).isEqualTo("user1");
	}
	...
	public void getPostsOfUser_whenMultipleUserExistWithMultiplePosts_receivePageWithMatchingPostsCount() {
		...
		ResponseEntity<TestPage<PostVM>> response = getPostsOfUser(userWithFivePosts.getUsername(),
				new ParameterizedTypeReference<TestPage<PostVM>>() {
				});
	...
	public void getOldPosts_whenThereArePosts_receivePageWithItemsBeforeProvidedId() {
		...
		ResponseEntity<TestPage<Object>> response = getOldPosts(fourth.getId(),
				new ParameterizedTypeReference<TestPage<Object>>() {
				});
		assertThat(response.getBody().getTotalElements()).isEqualTo(3);
	}
	...
	public void getOldPosts_whenThereArePosts_receivePageWithPostVMBeforeProvidedId() {
		...
		ResponseEntity<TestPage<PostVM>> response = getOldPosts(fourth.getId(),
				new ParameterizedTypeReference<TestPage<PostVM>>() {
				});
	...
	public void getOldPostsOfUser_whenUserExistAndThereArePosts_receivePageWithItemsBeforeProvidedId() {
		...
		ResponseEntity<TestPage<Object>> response = getOldPostsOfUser(fourth.getId(), "user1",
				new ParameterizedTypeReference<TestPage<Object>>() {
				});
		assertThat(response.getBody().getTotalElements()).isEqualTo(3);
	}
	...
	public void getOldPostsOfUser_whenUserExistAndThereArePosts_receivePageWithPostVMBeforeProvidedId() {
		...
		ResponseEntity<TestPage<PostVM>> response = getOldPostsOfUser(fourth.getId(), "user1",
				new ParameterizedTypeReference<TestPage<PostVM>>() {
				});
		assertThat(response.getBody().getContent().get(0).getDate()).isGreaterThan(0);
	}
	...
	public void getOldPostsOfUser_whenUserExistAndThereAreNoPosts_receivePageWithZeroItemsBeforeProvidedId() {
		...
		ResponseEntity<TestPage<PostVM>> response = getOldPostsOfUser(fourth.getId(), "user2",
				new ParameterizedTypeReference<TestPage<PostVM>>() {
				});
		assertThat(response.getBody().getTotalElements()).isEqualTo(0);
	}
	...
	public void getNewPosts_whenThereArePosts_receiveListOfItemsAfterProvidedId() {
		...
		ResponseEntity<List<Object>> response = getNewPosts(fourth.getId(),
	...
	public void getNewPosts_whenThereArePosts_receiveListOfPostVMAfterProvidedId() {
		...
		ResponseEntity<List<PostVM>> response = getNewPosts(fourth.getId(),
				new ParameterizedTypeReference<List<PostVM>>() {
				});
	...
	public void getNewPostsOfUser_whenUserExistAndThereArePosts_receiveListWithItemsAfterProvidedId() {
		...
		ResponseEntity<List<Object>> response = getNewPostsOfUser(fourth.getId(), "user1",
				new ParameterizedTypeReference<List<Object>>() {
				});
		assertThat(response.getBody().size()).isEqualTo(1);
	}
	...
	public void getNewPostsOfUser_whenUserExistAndThereArePosts_receiveListWithPostVMAfterProvidedId() {
		...
		ResponseEntity<List<PostVM>> response = getNewPostsOfUser(fourth.getId(), "user1",
				new ParameterizedTypeReference<List<PostVM>>() {
				});
		assertThat(response.getBody().get(0).getDate()).isGreaterThan(0);
	}
	...
	public void getNewPostsOfUser_whenUserExistAndThereAreNoPosts_receiveListWithZeroItemsAfterProvidedId() {
		...
		ResponseEntity<List<PostVM>> response = getNewPostsOfUser(fourth.getId(), "user2",
				new ParameterizedTypeReference<List<PostVM>>() {
				});
	...
	public void getNewPostCount_whenThereArePosts_receiveCountAfterProvidedId() {
		...
		ResponseEntity<Map<String, Long>> response = getNewPostCount(fourth.getId(),
	...
	public void getNewPostCountOfUser_whenThereArePosts_receiveCountAfterProvidedId() {
		...
		ResponseEntity<Map<String, Long>> response = getNewPostCountOfUser(fourth.getId(), "user1",
				new ParameterizedTypeReference<Map<String, Long>>() {
				});
		assertThat(response.getBody().get("count")).isEqualTo(1);
	}
	...
	public <T> ResponseEntity<T> deletePost(long postId, Class<T> responseType) {
		return testRestTemplate.exchange(API_V1_POSTS + "/" + postId, HttpMethod.DELETE, null, responseType);
	}

	public <T> ResponseEntity<T> getNewPostCount(long postId, ParameterizedTypeReference<T> responseType) {
		String path = API_V1_POSTS + "/" + postId + "?direction=after&count=true";
		return testRestTemplate.exchange(path, HttpMethod.GET, null, responseType);
	}

	public <T> ResponseEntity<T> getNewPostCountOfUser(long postId, String username,
			ParameterizedTypeReference<T> responseType) {
		String path = "/api/v1/users/" + username + "/posts/" + postId + "?direction=after&count=true";
		return testRestTemplate.exchange(path, HttpMethod.GET, null, responseType);
	}

	public <T> ResponseEntity<T> getNewPosts(long postId, ParameterizedTypeReference<T> responseType) {
		String path = API_V1_POSTS + "/" + postId + "?direction=after&sort=id,desc";
		return testRestTemplate.exchange(path, HttpMethod.GET, null, responseType);
	}

	public <T> ResponseEntity<T> getNewPostsOfUser(long postId, String username,
			ParameterizedTypeReference<T> responseType) {
		String path = "/api/v1/users/" + username + "/posts/" + postId + "?direction=after&sort=id,desc";
		return testRestTemplate.exchange(path, HttpMethod.GET, null, responseType);
	}

	public <T> ResponseEntity<T> getOldPosts(long postId, ParameterizedTypeReference<T> responseType) {
		String path = API_V1_POSTS + "/" + postId + "?direction=before&page=0&size=5&sort=id,desc";
		return testRestTemplate.exchange(path, HttpMethod.GET, null, responseType);
	}

	public <T> ResponseEntity<T> getOldPostsOfUser(long postId, String username,
			ParameterizedTypeReference<T> responseType) {
		String path = "/api/v1/users/" + username + "/posts/" + postId + "?direction=before&page=0&size=5&sort=id,desc";
		return testRestTemplate.exchange(path, HttpMethod.GET, null, responseType);
	}
	...
	public <T> ResponseEntity<T> getPosts(ParameterizedTypeReference<T> responseType) {
		return testRestTemplate.exchange(API_V1_POSTS, HttpMethod.GET, null, responseType);
	}

	private <T> ResponseEntity<T> postPost(Post post, Class<T> responseType) {
		return testRestTemplate.postForEntity(API_V1_POSTS, post, responseType);
	}
